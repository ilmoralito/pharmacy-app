...
const Item = {
  ...
  handleClick(event) {
    ...
    const { itemId, itemBalance, orderId } = Object.assign({}, target.dataset);

    if (target.textContent.trim() === "Editar") {
      const [quantityNode, purchasePriceNode, salePriceNode] = this.getNodes(
        target
      );
    ...
    if (target.textContent.trim() === "Confirmar") {
      const [quantity, purchasePrice, salePrice] = this.getInputsValue(target);
